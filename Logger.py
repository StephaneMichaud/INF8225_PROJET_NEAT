...
class Logger:
    ...
    def print_species_hierarchy(self, save=False):
        ...
        species_id_sorted_by_fitness = [specied_id for _, specied_id in sorted(
            zip(fitness, species_id_sorted_by_fitness))]
        species_id_sorted_by_gen = [specied_id for _, specied_id in sorted(
            zip(start_gen, species_id_sorted_by_fitness))]
