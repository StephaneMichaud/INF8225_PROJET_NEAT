...
class Logger:
    ...
    def print_species_hierarchy(self, save = False):
        ...
        for species_id, degree in degrees.items():
            ...
            size[species_id] = 1000*size[species_id]/max_avg_size
        ...
        for species_id, fitness in zip(species_id_unsorted,fitness):
            color_value[species_id] = ((fitness-min_fitness) / (max_fitness-min_fitness))**(1/2)
        ...
        print(size_list)
        nx.draw(G, with_labels=True, pos=pos, node_size = size_list,node_color = color,cmap=plt.cm.RdYlGn)
        plt.show()
