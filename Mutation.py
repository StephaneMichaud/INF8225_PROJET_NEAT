...
class MutationTracker:
    ...
    def __init__(self, input_size, output_size):
        # do this to help when checking structure similarity
        self.innov = input_size + output_size
        self.gen = 0
        self.add_node_track = {}
    ...
    def get_innov_nb_node_add(self, splitted_gene_innov_v):
        if splitted_gene_innov_v in self.add_node_track:
            ...
            self.add_node_track[splitted_gene_innov_v] = (
                self.innov+1, self.innov+2)
            self.innov += 2
...
def check_cyclic_connections(genome, s_node, t_node):
    ...
    if len(c_genes) == 0:  # if no connections
        return False
    # if dead end from either side
    if s_node < input_size or (t_node >= input_size and t_node < output_size):
        return False
...
def add_connection_mutation(genome, mutation_tracker, max_attempt=20, no_cycle=True):
    ...
    for attemp in range(0, max_attempt):  # to exit when no connections can be added
        if no_cycle:
            ...
            # to eliminate output nodes
            first_node = first_node + out_size if first_node >= in_size else first_node
        else:
            first_node = np.random.randint(0, n_size)
        last_node = np.random.randint(
            in_size, n_size)  # to eliminate input nodes
        ...
        if new_connection:
            ...
            innov_n = mutation_tracker.get_innov_nb_connect_add(
                first_node, last_node)
            genome.c_genes[first_node, last_node] = ConnectionGene(
                innov_n=innov_n, w_value=get_new_weight(), disable=False)
        else:
...
def add_node_mutation(genome, mutation_tracker):
    if len(genome.c_genes) == 0:
        return False, genome
    valid_connections = [connection for connection in list(
        genome.c_genes.keys()) if not genome.c_genes[connection].disable]
    if len(valid_connections) == 0:
        return False, genome

    old_input, old_output = valid_connections[np.random.randint(
        0, len(valid_connections))]
    innova, innovb = mutation_tracker.get_innov_nb_node_add(
        genome.c_genes[old_input, old_output].innov_n)
    new_node_nb = innova
    ...
    genome.c_genes[old_input, new_node_nb] = ConnectionGene(
        innov_n=innova, w_value=genome.c_genes[old_input, old_output].w_value, disable=False)
    # put weight value really close to 1
    genome.c_genes[new_node_nb, old_output] = ConnectionGene(
        innov_n=innovb, w_value=get_new_node_mutant_weight(), disable=False)
...
def alter_connection_mutation(genome, new_val_threshold_chance=0.1):
    ...
    if np.random.uniform(0, 1) < new_val_threshold_chance:
        ...
        # check if value are g odd
        genome.c_genes[old_input,
                       old_output].w_value += np.random.uniform(-0.001, 0.001)
    return True, genome
