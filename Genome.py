...
class Genome:
    ...
    def feed_forward(self, input, max_loop=1):
        ...
        while (len(hidden_next_it_list) > 0):
            ...
            # put zero if neuron not connected to any other neurons
            self.__n_values[temp] = neat_sigmoid(sum) if len(
                self.n_genes[temp].input_nodes) > 0 else 0
            # self.__n_values[temp] =  sum if len(self.n_genes[temp].input_nodes) > 0 else 0 #FOR DEBUG

            hidden_next_it_list.pop()

        # may need to put into a numpy array for better memory usage
        return list(self.__n_values.values())[self.input_size:self.input_size+self.output_size]
